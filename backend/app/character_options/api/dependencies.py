...
def get_class_skill_proficiencies_use_case(
    repo: SqlAlchemyCharacterClassRepository = Depends(get_class_repository),
) -> GetClassSkillProficienciesUseCase:
    return GetClassSkillProficienciesUseCase(repo=repo)


def get_class_starting_equipment_use_case(
    repo: SqlAlchemyCharacterClassRepository = Depends(get_class_repository),
) -> GetClassStartingEquipmentUseCase:
    return GetClassStartingEquipmentUseCase(repo=repo)


def get_class_spell_options_use_case(
    repo: SqlAlchemyCharacterClassRepository = Depends(get_class_repository),
) -> GetClassSpellOptionsUseCase:
    return GetClassSpellOptionsUseCase(repo=repo)
