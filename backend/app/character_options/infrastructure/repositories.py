...
from app.character_options.domain.models import (
    CharacterClass,
    CharacterSpecies,
    SkillOption,
    SpellEntry,
...
def _jstr(d: dict[str, object], key: str, default: str = "") -> str:
    """Safely read a string from a JSON dict."""
    v = d.get(key, default)
    return v if isinstance(v, str) else default


def _jint(d: dict[str, object], key: str, default: int = 0) -> int:
    """Safely read an int from a JSON dict."""
    v = d.get(key, default)
    return v if isinstance(v, int) else default


def _jlist(d: dict[str, object], key: str) -> list[object]:
    """Safely read a list from a JSON dict."""
    v = d.get(key, [])
    return v if isinstance(v, list) else []


def _parse_skill_options(value: object) -> tuple[SkillOption, ...]:
    """Parse JSON skill_options array into SkillOption domain objects."""
    if not isinstance(value, list):
        return ()
    result: list[SkillOption] = []
    for item in value:
        if isinstance(item, dict):
            result.append(
                SkillOption(
                    name=_jstr(item, "name"),
                    ability=_jstr(item, "ability"),
                )
            )
    return tuple(result)


def _parse_spell_options(value: object) -> tuple[SpellSection, ...]:
    """Parse JSON spell_options array into SpellSection domain objects."""
    if not isinstance(value, list):
        return ()
    sections: list[SpellSection] = []
    for section_raw in value:
        if not isinstance(section_raw, dict):
            continue
        spells_raw = _jlist(section_raw, "spells")
        spell_entries: list[SpellEntry] = []
        for spell_raw in spells_raw:
            if isinstance(spell_raw, dict):
                spell_entries.append(
                    SpellEntry(
                        name=_jstr(spell_raw, "name"),
                        description=_jstr(spell_raw, "description"),
                    )
                )
        sections.append(
            SpellSection(
                level=_jint(section_raw, "level"),
                label=_jstr(section_raw, "label"),
                choose_count=_jint(section_raw, "choose_count"),
                spells=tuple(spell_entries),
            )
        )
...
class SqlAlchemyCharacterClassRepository(CharacterClassRepository):
    ...
    def _to_domain(orm: CharacterClassORM) -> CharacterClass:
        ...
        skill_choose_count = (
            orm.skill_choose_count if isinstance(orm.skill_choose_count, int) else 0
        )
        return CharacterClass(
            ...
            theme=orm.theme,
            skill_options=_parse_skill_options(orm.skill_options),
            skill_choose_count=skill_choose_count,
            starting_equipment_items=tuple(_str_list(orm.starting_equipment_items)),
            spell_options=_parse_spell_options(orm.spell_options),
        )
