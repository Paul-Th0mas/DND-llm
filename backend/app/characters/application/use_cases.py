...
from app.character_options.domain.models import CharacterClass as CharacterClassType
...
class CreateCharacterUseCase:
    ...
    def execute(
        ...
        starting_equipment: list[str] | None = None,
        spells: list[str] | None = None,
    ) -> CharacterResponse:
        """
        Create and persist a new Character.

        Validates class/species existence, skill proficiency selection, and
        spell selection. Theme validation is the caller's responsibility.
        ...
        @param skill_proficiencies  - Skill names chosen; must match class skill_options.
        @param starting_equipment   - Equipment items; stored as-is, no validation.
        @param spells               - Spell names chosen; validated per spell section counts.
        @returns CharacterResponse DTO.
        @raises CharacterClassNotFoundError / CharacterSpeciesNotFoundError if not found.
        @raises InvalidCharacterError if skill or spell selection is invalid.
        """
        skills = skill_proficiencies if skill_proficiencies is not None else []
        equipment = starting_equipment if starting_equipment is not None else []
        chosen_spells = spells if spells is not None else []
        ...
        self._validate_skill_proficiencies(skills, cls)

        # --- Spell selection validation ---
        validated_spells = self._validate_spells(chosen_spells, cls)
    ...
    def _validate_skill_proficiencies(
        skills: list[str],
        cls: CharacterClassType,
    ) -> None:
        """
        Validate the submitted skill names against the class's skill_options.

        Normalizes submitted names to title-case before comparing.
        Raises InvalidCharacterError if any skill is unknown or the count is wrong.
        """
        normalized = [s.title() for s in skills]
        allowed = {opt.name.title() for opt in cls.skill_options}
        expected_count = cls.skill_choose_count

        if expected_count == 0:
            # Class has no selectable skills; list must be empty.
            if normalized:
                raise InvalidCharacterError(
                    f"Class '{cls.display_name}' has no selectable skills "
                    f"but {len(normalized)} were submitted"
                )
            return

        if len(normalized) != expected_count:
            raise InvalidCharacterError(
                f"Class '{cls.display_name}' requires exactly {expected_count} "
                f"skill(s) but {len(normalized)} were submitted"
            )

        unknown = [s for s in normalized if s not in allowed]
        if unknown:
            raise InvalidCharacterError(
                f"Unknown skill(s) for class '{cls.display_name}': {unknown}. "
                f"Allowed: {sorted(allowed)}"
            )

        logger.debug(
            "_validate_skill_proficiencies: class=%s skills=%s OK",
            cls.id,
            normalized,
        )
    ...
    def _validate_spells(
        ...
        """
        Validate the submitted spell names against the class's spell_options.

        Returns the normalized (title-case) spell list on success.
        Raises InvalidCharacterError if any spell is unknown or per-section
        counts don't match.

        Non-caster classes (spellcasting_ability is None) must submit [].
        """
        normalized = [s.title() for s in spells]

        # Non-casters: accept empty list, discard anything else silently by
        # enforcing [] (instructions say "accept [], ignore anything submitted").
        if cls.spellcasting_ability is None:
            return []

        # Build a flat name → section mapping for existence checks.
        spell_to_section: dict[str, int] = {}
        for sec in cls.spell_options:
            for sp in sec.spells:
                spell_to_section[sp.name.title()] = sec.level

        # Check all submitted spells exist.
        unknown = [s for s in normalized if s not in spell_to_section]
        if unknown:
            raise InvalidCharacterError(
                f"Unknown spell(s) for class '{cls.display_name}': {unknown}"
            )

        # Group submitted spells by section level.
        submitted_by_level: dict[int, list[str]] = {}
        for spell_name in normalized:
            level = spell_to_section[spell_name]
            submitted_by_level.setdefault(level, []).append(spell_name)

        # Verify per-section counts match choose_count.
        for sec in cls.spell_options:
            submitted_count = len(submitted_by_level.get(sec.level, []))
            if submitted_count != sec.choose_count:
                raise InvalidCharacterError(
                    f"Section '{sec.label}' (level {sec.level}) requires "
                    f"exactly {sec.choose_count} spell(s) but "
                    f"{submitted_count} were submitted"
                )

        logger.debug(
            "_validate_spells: class=%s spells=%s OK",
            cls.id,
            normalized,
        )
        return normalized
